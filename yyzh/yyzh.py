...
i = 1
for name in os.listdir(r'E:\wav'):
    print("%d %s 开始转换" % (i, name))
    ##音频分块识别
    r = sr.Recognizer()
    # for i in range(kn):
    try:
        with sr.WavFile(r'E:\wav\%s' % name) as source:
            audio = r.record(source)
            IBM_USERNAME = '41d330e2-d706-48ba-a3ea-27fb5bb912be'
            IBM_PASSWORD = '05WszP7hqHFV'
            text = r.recognize_ibm(audio, username=IBM_USERNAME, password=IBM_PASSWORD, language='zh-CN')
            print(text)
            open(r'C:\Users\dell\Desktop\tmp\%s.txt' % name, 'a+').write(text)
            time.sleep(5)
